"""
EX3501-T0 driver adapter (ZTE).

Connect: OPENINTERFACE + LOGINPANEL + SKIP_SCREENS + OPEN_SISTEM_PANELI.

Kaynak kod yapısına uyumlu adapter:
  - WAN tuple boyutu 2 (ip, sure) — 'status' alanı her zaman "N/A" yazılır.
  - IPv6 internet WAN'inde sadece IP döner.
  - Wi-Fi: SSID+Kanal sistem sayfasından, BW kablosuz sayfasından.
"""
import time
from . import scraper
...
def collect(driver, secilen: set) -> dict:
    ...
    # WAN — tuple 2 elemanlı (ip, sure); IPv6'da sadece ip string
    wan_keys = {
    ...
    # Wi-Fi: SSID+Kanal sistem sayfasından, BW kablosuz sayfasından (ayrı navigasyon)
    ...
    if {"download", "upload"} & secilen:
        ...
        # Orijinal akışta sent/recv değişkenleri sırayla download/upload anahtarlarına
        # atanıyor (row_list[dl, ul]) — bunu aynen koruyoruz.
